namespace Microsoft.Research.DynamicDataDisplay.Charts.Maps
{
	...
	public struct Degree : IComparable, IComparable<Degree>, IEquatable<Degree>
	{
		...
		public CoordinateType CoordinateType => coordinateType;
		...
		{
			return CreateLatitude(degrees, minutes, 0);
		}

		public static Degree CreateLongitude(int degrees, int minutes)
		{
			return CreateLongitude(degrees, minutes, 0);
		}
		...
		public int Degrees => Math.Sign(value) * (int)Math.Floor(Math.Abs(value));

		public int Minutes => (int)Math.Floor(TotalMinutes);
		...
		public int Seconds => (int)Math.Floor(TotalSeconds);
