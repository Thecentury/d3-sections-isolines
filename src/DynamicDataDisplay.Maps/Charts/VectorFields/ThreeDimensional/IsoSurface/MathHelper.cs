namespace Microsoft.Research.DynamicDataDisplay.Maps.Charts.VectorFields
{
	...
	internal static class MathHelper
	{
		...
		{
			return CalculateNormalToField(position.X, position.Y, position.Z, field);
		}
		...
		public static bool CheckPosition(Vector3D position, double[, ,] array, double missingValue)
		{
			...
			if (position.X >= 0 && position.X < i && position.Y >= 0 && position.Y < j && position.Z >= 0 && position.Z < k && !MissingCheck(position, array, missingValue))
				return true;
			return false;
