namespace Microsoft.Research.DynamicDataDisplay.Maps.Charts.Voronoi
{
	...
	public static class Output
	{
		...
		internal static void out_bisector(Edge e)
		{
			...
			if (!triangulate && !plot && !debug)
				Console.WriteLine("line {0} {1} {2}", e.a, e.b, e.c);
			if (debug)
				Console.WriteLine("line edgeNumber: {0}, {1}x+{2}y={3}, bisecting sites # {4} {5}", e.edgeNumber, e.a, e.b, e.c, e.reg[EndPoint.Left].SiteNumber, e.reg[EndPoint.Right].SiteNumber);
		...
		internal static void OutputEndPoint(Edge e)
		{
			...
			if (!triangulate && !plot)
			{
				Console.Write("endpoint edgeNumber: {0} ", e.edgeNumber);
				Console.Write("left: ");
		...
		{
			if (!triangulate && !plot && !debug)
				Console.WriteLine("vertex {0}, {1}", site.Coord.X, site.Coord.Y);
			if (debug)
				Console.WriteLine("vertex({0}) at {1}, {2}", site.SiteNumber, site.Coord.X, site.Coord.Y);
		...
		internal static void OutputSite(Site s)
		{
			...
			if (!triangulate && !plot && !debug)
				Console.WriteLine("site {0} {1}", s.Coord.X, s.Coord.Y);
			if (debug)
				Console.WriteLine("site ({0}) at {1} {2}", s.SiteNumber, s.Coord.X, s.Coord.Y);
		}

		internal static void out_triple(Site s1, Site s2, Site s3)
		{
			if (triangulate && !plot && !debug)
				Console.WriteLine("triple {0} {1} {2}", s1.SiteNumber, s2.SiteNumber, s3.SiteNumber);
			if (debug)
				Console.WriteLine("circle through left={0} right={1} bottom={2}", s1.SiteNumber, s2.SiteNumber, s3.SiteNumber);
		}
