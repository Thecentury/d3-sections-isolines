	...
	public class IntegerTicksProvider : ITicksProvider<int>
	{
		...
		public int MaxStep
		{
			...
			set
			{
				...
				{
					if (value < 0)
						throw new ArgumentOutOfRangeException("value", Exceptions.ParameterShouldBePositive);
		...
		private static int[] CreateTicks(double start, double finish, int step)
		{
			...
			int x = (int)(step * Math.Floor(start / step));
		...
		private static int[] tickCounts = { 20, 10, 5, 4, 2, 1 };
