namespace Microsoft.Research.DynamicDataDisplay
{
	...
	public abstract class Plotter : ContentControl
	{
		...
		protected PlotterLoadMode LoadMode => loadMode;
		...
		protected ResourceDictionary GenericResources => genericResources;
		...
		private bool isLoadedIntensionally;
		protected virtual bool IsLoadedInternal => isLoadedIntensionally || IsLoaded;
		...
		public VisualBindingCollection VisualBindings => visualBindingCollection;
		...
		bool executedWaitingChildrenAdding;
		...
		{
			get => performChildChecks;
			set => performChildChecks = value;
		}

		private IPlotterElement currentChild;
		protected IPlotterElement CurrentChild => currentChild;
		...
		protected virtual void OnChildAdded(IPlotterElement child)
		{
			if (child != null)
			{
				...
				try
				{
					...
					{
						throw new InvalidOperationException(Exceptions.PlotterElementAddedToAnotherPlotter);
					}
					...
					if (performChildChecks)
					{
						...
						{
							throw new InvalidOperationException(Exceptions.InvalidParentPlotterValue);
						}
					}
		...
		private UIElement CreateVisualProxy(IPlotterElement child)
		{
			if (visualBindingCollection.Cache.ContainsKey(child))
				throw new InvalidOperationException(Exceptions.VisualBindingsWrongState);
		...
		protected virtual void OnChildRemoving(IPlotterElement child)
		{
			if (child != null)
			{
				...
				try
				{
					...
					{
						throw new InvalidOperationException(Exceptions.InvalidParentPlotterValueRemoving);
					}
					...
					if (performChildChecks)
					{
						...
						if (child.Plotter != null)
						{
							throw new InvalidOperationException(Exceptions.ParentPlotterNotNull);
						}
					}
					...
					{
						throw new InvalidOperationException(String.Format(Exceptions.PlotterElementDidnotCleanedAfterItself, child));
					}
				}
				finally
				{
		...
		public UndoProvider UndoProvider => undoProvider;
