namespace Microsoft.Research.DynamicDataDisplay
{
	...
	public struct DataRect : IEquatable<DataRect>, IFormattable
	{
		...
		public DataRect(Point point1, Point point2)
		{
			...
			width = Math.Max(Math.Max(point1.X, point2.X) - xMin, 0);
			height = Math.Max(Math.Max(point1.Y, point2.Y) - yMin, 0);
		}
		...
		{
			if ((width < 0) || (height < 0))
				throw new ArgumentException(Exceptions.WidthAndHeightCannotBeNegative);
		...
		public void Intersect(DataRect rect)
		{
			...
			res.width = Math.Max(Math.Min(this.XMax, rect.XMax) - x, 0.0);
			res.height = Math.Max(Math.Min(this.YMax, rect.YMax) - y, 0.0);
			res.xMin = x;
			res.yMin = y;
		...
		public double YMin
		{
			...
			{
				if (this.IsEmpty)
					throw new InvalidOperationException(Exceptions.CannotModifyEmptyDataRect);
		...
		public double Width
		{
			...
			set
			{
				if (this.IsEmpty)
					throw new InvalidOperationException(Exceptions.CannotModifyEmptyDataRect);
				if (value < 0)
					throw new ArgumentOutOfRangeException(Exceptions.DataRectSizeCannotBeNegative);
		...
		public static DataRect Infinite => infinite;
		...
		public static DataRect Parse(string source)
		{
			...
			else
			{
				...
				else
				{
					...
					values = source.Split(new[] { ',', ' ' }, StringSplitOptions.RemoveEmptyEntries);
					rect = Create(
						Convert.ToDouble(values[0], cultureInfo),
						Convert.ToDouble(values[1], cultureInfo),
						Convert.ToDouble(values[2], cultureInfo),
