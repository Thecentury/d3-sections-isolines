	...
	public class ParallelClusteringFilter : GroupFilter
	{
		...
		protected internal override IEnumerable<IndexWrapper<Point>> Filter(IEnumerable<IndexWrapper<Point>> series)
		{
			...
			foreach (var wrapper in series)
			{
				...
				int clusterIndexX = Math.Max(0, Math.Min(xClustersNum - 1, (int)(Math.Floor((point.X - xMin) / clusterWidth))));
				int clusterIndexY = Math.Max(0, Math.Min(yClustersNum - 1, (int)(Math.Floor((point.Y - yMin) / clusterHeight))));
